...
angling_hotspots_wat$Permanent_ <- angNwat$Permanent_

## Cleanup and Export
# Identified hotspots with duplicate bobber ID, lake, and date combinations to get trips
angling_hotspots_wat$bdoID <- paste(angling_hotspots_wat$bobber_id, angling_hotspots_wat$Permanent_, angling_hotspots_wat$date, sep='x')
angling_hotspots_wat$uniq <- !duplicated(angling_hotspots_wat$bdoID)

# Table for first round of analyses
toa <- angling_hotspots_wat[angling_hotspots_wat$uniq==T,c("bobber_id", "latitude", "longitude", "date","Permanent_")]
